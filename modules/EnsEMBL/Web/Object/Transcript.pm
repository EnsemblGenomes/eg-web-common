...
sub count_duplication {
  my $self = shift;
  my $keys = shift;
  my $keys_type  = shift;

  my $hub = $self->hub;
  my @domains     = map  { @{$self->translation_object->get_all_ProteinFeatures($_)} } @$keys;
  my $all_elems = []; 
  my $duplications = 0;

  @domains = map {[ $_->analysis->db || $_->analysis->logic_name || 'unknown', $_ ]} @domains unless ($keys_type eq 'domain');
  foreach my $domain (@domains) {

       my $elem = {}; 
    
       if ($keys_type eq 'domain') {
	 ...
	 $elem = {
       ...
       my $dupl_flag = 0;
       foreach my $a (@{$all_elems}) {
         my $cnt_dupl  = 0;
         for my $key (keys %$elem) {
	   last unless exists $a->{$key};
	   last unless $a->{$key} eq $elem->{$key};	
           $cnt_dupl++;
         }
	 if ($cnt_dupl == (keys %$elem)) {
	     $dupl_flag = 1;
	     $duplications++;
             last;
	 } 
       }
       push @$all_elems, $elem unless $dupl_flag;
...
sub count_prot_domains {
  ...
  my $duplications = $self->count_duplication(\@domain_keys, 'domain');
  $duplications   += $self->count_duplication(\@other_keys,  'other' );

  $c += map { @{$self->translation_object->get_all_ProteinFeatures($_)} } @domain_keys, @other_keys;
  $c -= $duplications;
