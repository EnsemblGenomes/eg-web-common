...
<xsl:apply-templates select="*" />
  ...
  <div id="wide-footer">
    <div class="twocol-left left unpadded">Ensembl
      ...
      <a href="https://www.ebi.ac.uk/" style="white-space:nowrap">EBI</a>
      </div>
    <div class="twocol-right right unpadded">
      <a href="https://www.ensembl.org/info/about/index.html">About EnsemblGenomes</a> | 

      <a href="https://www.ensembl.org/info/about/contact/">Contact Us</a> | 
...
<xsl:template match="SEGMENT" mode="type">
  ...
  <tr>
    ...
    [<xsl:element name="a"><xsl:attribute name="href">
       /<xsl:value-of select="$species" />/Location/View?r=<xsl:value-of select="@id" />:<xsl:value-of select="@start" />-<xsl:value-of select="@stop" />
     </xsl:attribute>Location View</xsl:element>]
    [<xsl:element name="a"><xsl:attribute name="href">
       /<xsl:value-of select="$species" />/Location/Overview?r=<xsl:value-of select="@id" />:<xsl:value-of select="@start" />-<xsl:value-of select="@stop" />
