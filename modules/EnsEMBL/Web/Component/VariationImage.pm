...
## EG
  ...
  $gene_object->{_variation_zoom_slice} = $object->database('core')->get_SliceAdaptor->fetch_by_region( $gene_object->seq_region_type, $reg_name, $start, $end, 1);;
##
...
## EG  
  ...
  $gene_object->store_TransformedDomains($_, $start) for @domain_logic_names;    # Stores in $transcript_object->__data->{'transformed'}{'Pfam_hits'}
##  
...
## EG   
   ...
   $image_configs->{$_}->set_parameters({ container_width => $transcript_slice->length }); 
##
 }
...
## EG
  ...
  $gene_object->{_variation_zoom_slice} = undef;
##
  return $html;
}
