...
sub json_fetch_species {
  ...
  my %shown           = map { $params->{"s$_"} => $_ } grep s/^s(\d+)$/$1/, keys %$params;
  ...
  my %intra_species_by_region = (
    $chr => $hub->intra_species_alignments('DATABASE_COMPARA', $prodname, $object->Obj->{slice} || $chr),
  );

  foreach my $alignment (grep $start < $_->{'end'} && $end > $_->{'start'}, @{$intra_species_by_region{$chr}}) {
  ...
  foreach (grep !$available_species{$_}, keys %shown) {
    my ($sp, $target) = split '--';
    if ($sp eq $prodname) {
      $intra_species_by_region{$target} ||= $hub->intra_species_alignments('DATABASE_COMPARA', $prodname, $target);
      $included_regions{$target} = $intra_species_by_region{$target};
    }
  }
