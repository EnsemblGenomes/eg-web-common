...
#    warn "Find $relation relation between $param\n"; 

    my $sql = qq{SELECT r.child_term_id, r.parent_term_id, rt.name
FROM relation r
LEFT JOIN relation_type rt  USING (relation_type_id)
WHERE rt.name NOT IN ('is_a', 'part_of')
AND child_term_id IN ($param)
AND parent_term_id IN ($param)
};

# warn "SQL : [$sql]\n";

    my $sth2 = $this->prepare($sql);
    $sth2->execute();
