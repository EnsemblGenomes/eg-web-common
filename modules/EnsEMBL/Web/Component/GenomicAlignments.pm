...
package EnsEMBL::Web::Component::GenomicAlignments;
...
sub align_table {
  ...
  foreach my $aln ( @$data ){
    ...
    push(@rows,
      {
      'species' => $loc ? $self->_link(sprintf('<em>%s</em> : <em>%s</em>',$sp1name,$sp2name),$self->hub->url({ 'species'=>$sp1, 'type'=>'Location', 'action'=>'Compara_Alignments/Image','r'=>$loc,'align'=>$aln->{'id'} })):$aln->{'species'},
      'type' => $aln->{'type'}
      }
    );
  }
