...
Ensembl.Panel.TaxonSelector = Ensembl.Panel.extend({
  constructor: function (id, params) {
    ...
    this.isTools  = params.isTools ? true : false ;
  ...
  init: function () {
    ...
    // override the default modal close handler
		if (this.isTools) {
		  $('.modal_close').unbind().bind('click', function () {
		    ...
		    return panel.updateConfigurationTools();
		  });
		} else {
		  $('.modal_close').unbind().bind('click', function () {
    ...
    $.getScript(panel.dataUrl, function() {
	    ...
	    panel.elLk.tree.dynatree({
				...
				checkbox: true,
				selectMode: (panel.multiSelect) ? 3 : 1,
				activeVisible: true,
			...
			//Hiding checkbox for clades/folder if there is no multiselection
			if(!panel.multiSelect) {
				panel.elLk.tree.dynatree("getRoot").visit(function(dtnode){
  ...
  updateConfigurationTools: function() {
 		var panel = this;
    var items = panel.getSelectedItems();
