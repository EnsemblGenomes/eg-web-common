...
sub ontology_table {
    ...
    my $columns       = [   
	{ key => 'ancestor_chart',  title => 'Chart',    width => '5%', align => 'centre' },
	{ key => 'go',              title => 'Accession',         width => '5%',  align => 'left'   },
	{ key => 'description',     title => 'Term',              width => '20%', align => 'left'   },
	{ key => 'evidence',        title => 'Evidence',          width => '3%',  align => 'center' },
	{ key => 'source',          title => 'Annotation Source', width => '24%', align => 'center' },
	{ key => 'goslim_goa_acc',  title => 'GOSlim Accessions', width => '5%', align => 'centre' },
	{ key => 'goslim_goa_title',title => 'GOSlim Terms',      width => '20%', align => 'centre' },
	];
    ...
    my $table = $self->new_table($columns, [], 
				 {
				     code=>1,data_table=>1,id=>'ont_table',toggleable=>0,
				     class => '',
...
sub ontology_chart {
    ...
    if ($bm_filter &&  $species_defs->GENOMIC_UNIT !~ /bacteria/) {
