...
## EG - use species selector instead of simple dropdown
   my $select = $form->add_element(
    ...
    'name'    => 'species',
    'label'   => "Species",
    'values'  => @species <= 100 ? \@species : [{ value => $current_species, caption => $hub->species_defs->species_display_label($current_species) }],
...
## /EG   
  ...
  $html .= $render;
#  $html = qq{<select id="my-select" class="species-selector" data-placeholder="Select a species..."><option value="escherichia_coli_str_k_12_substr_mg1655">Escherichia coli str. K-12 substr. MG1655, (TaxID 511145)</option></select>} . $html;
  return $html;
