...
my ($force_update,$check_registry,$site,$xml,$override_authority,$print_coords) = (0,0,'',0);
GetOptions(
  "force",  \$force_update,
...
## EG  
  next if $sp =~ /^Triticum_aestivum$/i; # do not include wheat
##
  ...
  for my $coord_type ('ensembl_gene', 'ensembl_peptide') {
    my $cxml = _coord_system_as_xml($coord_type, '', $sp, $taxid);
    print "$cxml\n" if $print_coords;
      
    unless (exists $das_coords->{$sp}{$coord_type}) {
      # Add to the registry and check it came back OK
      my $tmp = _get_das_coords($cxml, $coord_type);
      if (! $check_registry) {
...
sub _coord_system_as_xml {
  ...
  $species = $species_defs->get_config($cs_species, 'SPECIES_SCIENTIFIC_NAME');
  $version = undef;
...
## EG  
  $type =~ s/^(Supscaffold|Superscaffold)$/Scaffold/i; # hack for types not in registry yet
##  
