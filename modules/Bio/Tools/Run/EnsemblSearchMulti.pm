...
#	my( $sp, $dbprefix, $db_key ) = $db =~ /^([\w|_]+)_(cdna_|dna_rm_|dna_|ncrna|pep_)([A-Z_]+)$/i;
        my ( $sp, $dbprefix, $db_key ) = $db =~ /^([\w|_]+)_(cdna|dna_rm|dna|ncrna|pep|latestgp)(_\w+)?$/i;

warn qq{ \n\n\ndb: $db \n dbprefix: $dbprefix \n db_key: $db_key \n sp: $sp \n\n\n};

	$db_key = $dbprefix.$db_key;
